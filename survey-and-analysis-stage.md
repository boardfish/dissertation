...
Commercial escape rooms are more regularly built as permanent fixtures to be attempted multiple times in a day, and their puzzles usually need to be reset back to their initial state after completion (@liam2019). The requirement for multiple groups to attend an escape room at once can be present in the classroom, particularly if an escape room is part of a planned lesson with limited time. This creates a double-edged sword of a requirement: the available puzzles should not need to be reset between attempts. This comes at the cost of player immersion in commercial escape rooms, as the puzzle will need to be simpler and less physically involved. With the aforementioned statistic on the physicality of escape rooms, the requirement is often dismissed for its faults. A study by @lopezuse organised its puzzles in a manner whereby ordering is not necessary, allowing multiple groups to attend the room at once without the need for puzzles to be reset between attempts by separate groups. However, some varieties of escape room can apply this - portable escape rooms are one such variety, e.g. Tuzak, an Istanbul company developing escape games (@gunduz2018preventing)

Application of technology in escape rooms comes with some uncertainty, and a break in the flow of the escape room experience can shatter participants' immersion and lead to bad reviews (@liam2019). This also creates some difficulty when it comes to visibility - unless monitors are suitably placed and large enough to be viewed by a full party, the whole team may not be able to interface with a puzzle that applies technology. This is particularly an issue if a single typical workstation is set up - @nicholson2015peeking warns of the danger of removing just one player from the "mental space".


@liam2019 theorises that VR escape rooms such as <!-- TODO: cite Exit Berlin --> Exit Berlin may be the next stage for the industry, which would allow immersive rooms to be created while effectively eliminating the issue of resetting the room as above. This would bring the escape room cycle full circle, reincarnating the modern wave of physical escape rooms in the digital form that inspired them.

<!-- TODO: discuss portable rooms -->
<!-- in education -->
